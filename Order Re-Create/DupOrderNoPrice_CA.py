...
usdata = pds.read_table(r'<insert_file_path>\4_11All.txt')
usdatastring=usdata["OrdNbr"].str.cat(sep=' ')
sql_usdata = f"select h.ordnbr as OldOrdNbr, h.CustId,h.User2, h.CustOrdNbr,so.ChainDisc,h.User3,h.User1,h.AuthNbr,h.TermsID,so.SiteID,so.Invtid, so.QtyOrd,so.QtyCloseShip,so.QtyOrd-so.QtyCloseShip as QtyToOrd, h.ShipToID, so.TaxCat from soheader h join soline so on so.OrdNbr = h.OrdNbr where so.QtyOrd<>so.QtyCloseShip and h.OrdNbr in ({usdatastring}) order by h.OrdNbr Asc"
...
df.to_csv(path_or_buf=r'<insert_file_path>\4_11All.csv', index=False)
...
r = 0
c = 0
with open(r'<insert_file_path>\4_11AllImport.csv','w', newline = '') as file:
    ...
    for ind in df.index:
        ...
        ShipToID = df.iat[r,c+14] 
        TaxCat = df.iat[r,c+15] 
...
# Look at adding TaskID & ProjectID
        ...
        if (PrevOrdNbr == OldOrdNbr):
            ...
            writer.writerow(["LEVEL0",CustID,CustOrdNbr,User1,TermsID,User3,OldOrdNbr,AuthNbr,ShipToID])
            writer.writerow(["LEVEL1",InvtId,QtyToOrd,ChainDisc,SiteId,TaxCat])
