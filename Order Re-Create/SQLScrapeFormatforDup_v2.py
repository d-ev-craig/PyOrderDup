...
usdata = pds.read_table(r'<insert_file_path>\Rd2 Open Shippers.csv')
usdatastring=usdata["OrdNbr"].str.cat(sep=' ')
sql_usdata = f"select h.OrdNbr, h.CustID,h.user2, h.CustOrdNbr,l.ChainDisc,h.user3,h.user1,h.authnbr,h.termsid, l.SiteID, l.invtid, l.QtyOrd, l.QtyShip, l.qtycloseship, l.QtyOrd - l.QtyCloseShip as QtyToOrd,l.curyslsprice from soheader h left join soline l on l.OrdNbr = h.OrdNbr where l.QtyOrd <> l.QtyCloseShip and h.OrdNbr in {usdatastring}"
...
df = pds.read_table(r'<insert_file_path>Rd2 Open Shippers.csv')
df_obj = df.select_dtypes(['object'])
df[df_obj.columns] = df_obj.apply(lambda x: x.str.strip())

df.to_csv(path_or_buf=r'<insert_file_path>Rd2 Open Shippers.csv', index=False)
...
r = 0
c = 0
with open(r'<insert_file_path>\TIRd2.csv','w', newline = '') as file:
    ...
    for ind in df.index:
        ...
        if (PrevOrdNbr == OldOrdNbr):
            writer.writerow(["LEVEL1",InvtId,Price,QtyToOrd,ChainDisc,SiteId])
        else:
