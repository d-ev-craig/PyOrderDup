...
usdata = pds.read_table(r'<insert_file_path>\Rd2DataQuery.txt')
usdatastring=usdata["OrdNbr"].str.cat(sep=' ')
sql_usdata = f"select h.ordnbr as OldOrdNbr, h.CustId,h.User2, h.CustOrdNbr,so.ChainDisc,h.User3,h.User1,h.AuthNbr,h.TermsID,so.SiteID,so.Invtid, so.QtyOrd,so.QtyShip,so.QtyCloseShip,so.curyslsprice from soheader h join soline so on so.OrdNbr = h.OrdNbr where so.qtycloseship = '0' and so.qtyopenship <> '0' and h.OrdNbr in {usdatastring}"
