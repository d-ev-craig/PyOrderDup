...
usdata = pds.read_table(r'<insert_file_path>\DupData2.csv')
usdatastring=usdata["OrdNbr"].str.cat(sep=' ')
sql_usdata = f"select h.OrdNbr, h.CustID,h.user2, h.CustOrdNbr,l.ChainDisc,h.user3,h.user1,h.authnbr,h.termsid, l.SiteID, l.invtid, l.QtyOrd, l.QtyShip, l.QtyOrd - l.QtyShip as QtyToOrd,l.curyslsprice from soheader h left join soline l on l.OrdNbr = h.OrdNbr where l.QtyOrd <> l.QtyShip and h.OrdNbr in {usdatastring}"
...
df.to_csv(path_or_buf=r'<insert_file_path>\NewOrdersFormat2.csv', index=False)
...
r = 0
c = 0
with open(r'<insert_file_path>\TIFormatted2.csv','w', newline = '') as file:
